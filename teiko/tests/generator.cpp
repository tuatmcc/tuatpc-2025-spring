...
int main(int argc, char *argv[])
{
   ...
   for (int t = 1; t <= 5; t++)
   {
      ...
      while (true)
      {
         ...
         for (auto line : s)
         {
            ...
            {
               auto sub = line.substr(start, "^v^v^v"s.size());
               if (sub == "^v^v^v")
               {
                  r_cnt++;
               }
         ...
         if (hw - hw_cnt >= 0 && cases.size() + 1 <= Q_MAX && r_cnt <= R_NUM_MAX)
         {
            cases.push_back(s);
         }
         else
   ...
   cout << "DEBUG 07 07_handmade_01" << std::endl;
   {
      ...
      std::vector<std::string> hacks = {
          "(R(RRRRR)(RR[RR](RRRR[RR]RR)([RRR(RR)R][RR]RR)))",
          "([RRRRRR]RR[RRRRR]R(RRRRRR)[RRR]RR(RRRR)R)",
          "(([RR]R[RR])[RR]R([RR][RR]R)((RR)[RR](RR[RR]R)RR)[RR])",
          "([(R(RR[RR(RR[RR([RRR]RR(R[RR]R)RR)RR]R)RR]R)R)RR]R)",
          "[[(RR)R](RR)]",
          "[(RRR)[RR]]",
          "[R(R[R(RR)])]",
          "[R[R[R[R[R[RR]]]]]]"};
      of << hacks.size() << std::endl;
         ...
         for (int i = 1; i <= n - 2; i++)
         {
            ...
            for (int op = 0; op <= i; op++)
            {
               ...
               auto rec = [&st, br, i](auto &&self, int op, int cl, int r, std::string s) -> void
               {
                  ...
                  {
                     auto t = s;
                     if (i == 1 && ((s.back() == '(' || s.back() == '[') && (s.back() == ')' || s.back() == ']')))
                        return;
                     if (is_ok_brackets(t))
                     {
